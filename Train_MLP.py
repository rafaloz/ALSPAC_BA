...
# Cargo los datos
datos_all = pd.read_csv('/home/rafa/PycharmProjects/JoinData_FastSurfer_V2/datos/datos_completos_23_5_2024/FastSurfer_data_V2_morfo_con_WAND.csv')
...
# save dir
save_dir = '/home/rafa/PycharmProjects/JoinData_FastSurfer_V2/modelos/modelos_con_todo/modelo_morfo_100_Cardiff_7_90_noBadFeatures/'
...
datos_todos = datos_todos.sample(frac=1, random_state=10).reset_index(drop=True)
datos_train_all, datos_test_all = train_test_split(datos_todos, test_size=0.1, random_state=10)
...
for j in [100]:
    ...
    datos_train, datos_test = datos_train_all, datos_test_all
    datos_train, datos_val = train_test_split(datos_train, test_size=0.11, random_state=10)
    ...
    datos_train = datos_train.drop(['ID', 'Bo', 'Escaner', 'DataBase', 'Edad', 'Patologia'], axis=1)
    datos_val = datos_val.drop(['ID', 'Bo', 'Escaner', 'DataBase', 'Edad', 'Patologia'], axis=1)
    datos_test = datos_test.drop(['ID', 'Bo', 'Escaner', 'DataBase', 'Edad', 'Patologia'], axis=1)
    ...
    # Tab CNN
    model = Perceptron()
    MAEs_and_rs_perceptron_test = execute_in_val_and_test_NN(X_train, edades_todos_train, X_val, edades_todos_val,
                                                             X_test, edades_todos_test, listas_perceptron, model, j,
                                                             save_dir, i)
    Results_dataframe_perceptron_test = pd.concat([MAEs_and_rs_perceptron_test, Results_dataframe_perceptron_test],
                                                  axis=0)
