...
def harmonizationLearn(data, covars, eb=True, smooth_terms=[],
    ...
    if orig_model is None:
        ...
        isTrainSiteColumnsOrig = np.where(
            (pd.DataFrame(model['SITE_labels']).isin(np.unique(covars['SITE'])).values).flat)
    ...
    if smooth_model['perform_smoothing']:
        ...
        if orig_model is None:
            bs = BSplines(X_spline, df=[10] * len(smooth_cols), degree=[3] * len(smooth_cols),
                          knot_kwds=[{'lower_bound': smooth_term_bounds[0],
                                      'upper_bound': smooth_term_bounds[1]}] * len(smooth_cols))
    ...
    if orig_model is None:
        s_data, stand_mean, var_pooled, B_hat, mod_mean, grand_mean = standardizeAcrossFeatures(data, design, info_dict,
                                                                                                smooth_model)
        ...
        bayes_data = adjust_data_final(s_data, design, gamma_star, delta_star, stand_mean, mod_mean, var_pooled,
                                       info_dict, data)
        ...
        if batch_levels.size == 0:
            ...
            info_dict_train['batch_info'] = [list(np.where(covars[isTrainSite, batch_col] == idx)[0]) for idx in
                                             batch_levels]
            tmp = np.concatenate(
                (np.zeros(shape=(info_dict['n_sample'], len(model['SITE_labels']))), design[:, len(batch_labels):]),
                axis=1)
            s_data_train, stand_mean_train, v_pool, mod_mean = applyStandardizationAcrossFeatures(data[:, isTrainSite],
                                                                                                  tmp[isTrainSite, :],
                                                                                                  info_dict_train,
                                                                                                  model)
            design2 = tmp.copy()
            design2[:, isTrainSiteColumnsOrig[0]] = design[:, isTrainSiteColumns[0]]
            bayes_data_train = adjust_data_final(s_data_train, design2[isTrainSite, :], model['gamma_star'],
                                                 model['delta_star'], model['mod_mean'], model['var_pooled'],
                                                 info_dict_train, data[:, isTrainSite])
            # transpose data to return to original shape
        ...
        if batch_levels.size == 0:
            ...
            s_data_test, stand_mean_test, v_pool, mod_mean = applyStandardizationAcrossFeatures(data[:, ~isTrainSite],
            ...
            for i in range(info_dict_test['n_batch']):
                diff_mean = np.mean(data[:, info_dict_test['batch_info'][i]] - np.dot(
                    design[info_dict_test['batch_info'][i], info_dict['n_batch']:],
                    model['B_hat'][len(model['SITE_labels']):, :]).T, axis=1)
                betas.append(diff_mean)
            new_betas = np.array(betas)
            model['B_hat'] = np.concatenate((model['B_hat'][:len(model['SITE_labels']), :], new_betas,
            ...
            bayes_data_test = adjust_data_final(s_data_test, design_tmp[~isTrainSite, :], gamma_star, delta_star,
                                                stand_mean_test, model['mod_mean'], model['var_pooled'], info_dict_test,
                                                data[:, ~isTrainSite])
            # transpose data to return to original shape
...
def standardizeAcrossFeatures(X, design, info_dict, smooth_model):
    ...
    if smooth_model['perform_smoothing']:
        ...
        for i in range(0, X.shape[0]):
            ...
            gam_bs.alpha = gam_bs.select_penweight_kfold(k_folds=2, k_grid=5)[0]
...
def fitLSModelAndFindPriors(s_data, design, info_dict, eb=True):
    ...
    gamma_hat = np.array(
        np.dot(np.dot(np.linalg.inv(np.matrix(np.dot(batch_design.T, batch_design))), batch_design.T), s_data.T))
...
def _combat_diagnostics_soft(
    ...
    for b in range(n_batch):
        ...
        # momentos teóricos para CV
        mean_th = scale_mle / (a_mle - 1)  # α>1 garantizado en MLE
        var_th = scale_mle ** 2 / ((a_mle - 1) ** 2 * (a_mle - 2)) if a_mle > 2 else np.inf
        cv_th = np.sqrt(var_th) / mean_th if np.isfinite(var_th) else np.inf
        cv_emp = np.std(d, ddof=1) / np.mean(d)
        is_delta_ok = abs(cv_emp - cv_th) / cv_th < cv_tol

        # ---------- logging -------------------------------------------------
        print(
            f"Batch {b:2d} | γ̂: W={w:6.3f} p={p_w:.1e} "
            f"|Z_skew|={abs(z_skew):4.2f} |Z_kurt|={abs(z_kurt):4.2f} "
            f"→ {'OK' if is_gamma_ok else 'flag'}\n"
            f"          δ̂: D={D:5.3f} p={p_ks:.1e} "
            f"CV_emp={cv_emp:5.3f} CV_th={cv_th:5.3f} "
            f"→ {'OK' if is_delta_ok else 'flag'}"
        )
        ...
        if not is_delta_ok:
            problems.append(f"Batch {b}: δ̂ CV off by >{cv_tol:.0%}")
